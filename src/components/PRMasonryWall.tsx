...
export default async function PRMasonryWall() {
  const events = await getEventData();
  const items: WallItem[] = events
  ...
  if (items.length === 0) {
    return (
      <div className="w-full py-10 text-center text-gray-400">目前沒有可顯示的 PR 事件</div>
    );
  }
        ...
        {[...laneItems, ...laneItems].map((item, i) => {
          ...
          return (
            <div key={`${laneIndex}-${item.id}-${i}`} className="inline-block">
              <div className="card-drift bg-gray-800/50 p-4 rounded-lg border border-cyan-500/30 backdrop-blur-sm shadow-sm hover:border-cyan-400/60 transition-colors w-72 overflow-hidden whitespace-normal"
  ...
  return (
    <div className="space-y-6">
      {renderLane(laneA, 30, 0)}
      {renderLane(laneB, 36, 1)}
    </div>
  );
}
